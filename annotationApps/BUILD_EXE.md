...
**基本的に Python コード（`.py`）を変更してもビルドは不要です！**

| 変更内容 | ビルドは必要？ | 理由 |
|---------|:---:|------|
| **Pythonコードの変更** | **不要** 🙅‍♂️ | EXEは単にPythonを呼び出しているだけだから |
| **Batファイルの中身変更** | **不要** 🙅‍♂️ | 呼び出すファイル名が変わらなければOK |
| **Batファイル名の変更** | **必要** 🙆‍♂️ | ランチャー内でファイル名を指定しているため |
| **アイコンの変更** | **必要** 🙆‍♂️ | EXEに埋め込まれている画像を変えるため |
| **コンソール表示設定の変更** | **必要** 🙆‍♂️ | `--noconsole` オプションを変えるため |
